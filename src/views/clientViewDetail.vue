  ...
  methods: {
    ...
    // },
     async getAllThesis() {
      ...
      this.thesis2 = response2.data;
      this.total = response2.data.length;
      for (let i = 0; i < this.total; i++) {
        this.image.push(
          "data:" +
            this.thesis2[i].thesis.contentType +
            ";base64," +
            this.thesis2[i].thesis.data
        );
      }
      let y = 0;
      this.thesis2.forEach((element) => {
        (element.image = this.image[y]), y++;
      });
    },
    async update() {
      axios
